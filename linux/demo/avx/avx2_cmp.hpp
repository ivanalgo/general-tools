...
struct AVX2_CMP {
	...
	{
		return std::array{
			OpEntry2<ARG1_TYPE, ARG2_TYPE, OUTPUT_TYPE>{ "cmp_gt", "out[i] = a[i] > b[i] ? -1 : 0", avx_gt, sisd_gt },
			OpEntry2<ARG1_TYPE, ARG2_TYPE, OUTPUT_TYPE>{ "cmp_eq", "out[i] = a[i] == b[i] ? -1 : 0", avx_eq, sisd_eq },
		};
	}
};
